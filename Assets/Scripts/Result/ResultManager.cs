...
{
    public Text rankText;  // ランク表示用テキスト
    public Text Score1Text;  // ゲーム1のスコア表示用テキスト
    public Text Score2Text;  // ゲーム2のスコア表示用テキスト
    public Text Score3Text;  // ゲーム3のスコア表示用テキスト
    ...
    private void DisplayRankAnimation(int game1Score, int game2Score, int game3Score)
    {
        ...
        // ランク表示の初期設定
        rankText.text = rank;
        rankText.transform.localScale = Vector3.one * 1.5f;  // 初期サイズを大きめに設定
        rankText.color = new Color(rankText.color.r, rankText.color.g, rankText.color.b, 0);  // 完全に透明に設定
